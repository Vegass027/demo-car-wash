...
| 15b | **Фаза 2 — Slice #3b (staff booking mutations)** | ✅ **Done** — `api/staff.ts` +21 actions (11 carwash + 10 tire = 21), 2 helpers `api/_lib/{booking-services,earnings}.ts` (NOT serverless, не считается к 12/12), migration `008_atomic_create_staff_carwash_booking_rpc.sql` (advisory lock + recheck overlap + INSERT atomic) + rollback + hotfix `009_drop_old_overload.sql`. **54 assert() PASS / 0 FAIL** на prod alias (`test-staff-booking-endpoints.mjs`). Vercel: 6 deploys в Slice #3b (initial + 4 hotfixes за contract-bugs + final fix). Bugs caught at runtime: ALLOWED_ACTIONS set не обновлён на первом deploy → dispatcher 404 для всех #3b actions; .or(id.in.(${idList})) одинарные кавычки → UUID parse error; два overloads RPC (text[]/jsonb) после p_services type fix → ambiguity; JSONB возвращается как raw JSON string через supabase-js → нужен JSON.parse в lockCarwashBooking/lockTireBooking; tire_bookings НЕ имеет `notes` column (только в TS-интерфейсе) → cancel_reason dropped, patch.notes в DISALLOWED_NAMES; tire_payment_method enum шире чем carwash (включает 'Наличные' с 'е' + 'Яндекс') → отдельный TIRE_PAYMENT_METHODS. Все #3b actions идемпотентны где appropriate (mark-paid/mark-ready/cancel). Pre-Step-2 условия все закрыты: closed_box logic mirrors `lib/api/bookings.ts:172-189`, ACL блок в migration, antifreeze allowlist verified, E7/E8 dispatcher mapping строгий `===` |
