...
| 21 | **Phase 2.4 / Slice #3c (Category A writes) deployed** | ✅ **Done** — 15 dispatcher actions registered in `api/staff.ts` + 15 typed wrappers in `lib/api/staff-actions.ts`. 14 frontend callsites ported (Admins.tsx ×8, SalarySettingsWizard ×1, CompanySettingsWizard ×2, manageBalance.ts ×5). `finishAdminShift` JS wrapper DELETED (dead code, 0 external callers). Migration `017_category_a_rls_split_owner_all_admin_select.sql` applied to test DB (4 sections: grants + drop policies + new RLS + REVOKE EXECUTE on `start_admin_shift`). **Authorization matrix** (verified): 4 admin-or-owner (#4, #9, #10, #12) + 11 owner-only (with explicit justification per slice). **Test cluster**: factual baseline pre-Slice-3c was **80 PASS / 0 FAIL** (entry 20e). After Slice-3c adds: **+31 NEW asserts** = **111 PASS / 0 FAIL** total. Bug caught during cycle: `balance_after` column is NOT NULL but my dispatcher made it optional — fixed in commit `7bfeb34` by making it required + numeric. **Browser smoke**: 10/10 — 4 admin-or-owner actions succeeded for admin role (start-admin-shift, 3× create transaction); 6 owner-only actions correctly returned `owner_only_required` for admin role; anon direct read still blocked (`42501 permission denied for table admins`). **Cleanup**: 0 test rows, 0 smoke rows, 0 recent salary_transactions. **Critical**: `finish_admin_shift` RPC does NOT exist in pg_proc (verified), so no REVOKE needed. Production deployment remains gated (separate explicit question). |
