...
| 20c | **OD#11 + Phase 2.4 (add_worker_earnings REVOKE)** | ✅ **Done** — Architectural decision: handleMarkAsReady's non-antifreeze branch's `addWorkerEarningsForBooking` calls were REDUNDANT with `markStaffReady` server-side. Both paths produced identical 1-ledger-row outcome; client-side was leftover from pre-Slice-#3b era. Option 1 chosen: delete redundant calls. **(1) App.tsx** — handleMarkAsReady simplified: both antifreeze-only and non-antifreeze branches converge on single `markStaffReady` call. Antifreeze-only branch preserves pre-emptive `updateWorker(status:'available')` (antifreeze services don't go through regular services path that triggers worker status update elsewhere). After markStaffReady: local state refreshed via `getWorkers()` + `setWorkers()` (same pattern as handleMarkTireBookingAsReady). **(2) lib/api/workers.ts** — `addWorkerEarningsForBooking` function fully removed (-82 LOC including JSDoc). Not deprecated, eliminated to prevent future callers from re-introducing the bypass. **(3) Migration `015_revoke_add_worker_earnings_od11.sql`** — REVOKE EXECUTE FROM PUBLIC + FROM anon + FROM authenticated on `add_worker_earnings`. **Bug caught**: this RPC had EXPLICIT `anon=X/postgres, authenticated=X/postgres` grants (not just PUBLIC like the others), so both REVOKE targets were needed. Verified via proacl: `{postgres=X/postgres, service_role=X/postgres}` only. After fix: anon/authenticated SET ROLE + call → permission_denied ✓; service_role EXEC preserved ✓. **OD#11 documentation** in App.tsx handleMarkAsReady inline comment: «delegates earnings entirely to mark-staff-ready server-side path. No client-side RPC bypass. R5 race cluster covers the full RPC+ledger idempotency. Antifreeze-only bookings DO generate earnings — server's `calculateWorkerEarnings` uses `booking.price + discount × solo/pair rate` (no antifreeze-specific branch, antifreeze services count toward commission just like regular ones).» **Browser smoke** (curl + JWT demo_admin): non-antifreeze booking price=500 → earnings=200 (40% solo) ledger row ✓; antifreeze-only booking price=150 → earnings=60 (40% solo) ledger row ✓. **Reference values for future regression checks**: (a) non-antifreeze booking with solo worker + price=500 → 200 RUB EARNING (40% solo commission); (b) antifreeze-only booking with solo worker + price=150 → 60 RUB EARNING (40% solo commission, antifreeze counted like regular service). These numbers are stable as long as `salary_settings.worker_solo_commission=0.4` and antifreeze-org price=150 (verify in DB if regression suspected). Both flows confirmed working end-to-end without client-side RPC. Test cluster: 73/73 PASS pre- and post-REVOKE. Cleanup verify: 0 test rows, 0 smoke rows, 0 recent salary_transactions. Vercel deploys: 1 for App.tsx + workers.ts commit (bo9k43usz). |
| 20e | **Phase 2.1a — change-password dispatcher (functional done, push blocked)** | ⚠️ **State**: Server-side **deployed** (commit `5d55f5e`, Vercel auto-deploy ~16min ago, prod alias `demo-car-wash.vercel.app` running). Local un-pushed commits: `3e2a068` (validation error code unification), `fa25372` (migration 016 file), `7fe4e94` (E1-E7 tests). REVOKE migration 016 **applied directly to test DB via psql** (work-around for git push blocker). **(1) Server**: `api/staff.ts` added `changeStaffPasswordAction` with `requireStaff` guard, `p_user_id` server-stamped from `claims.profile_id` (rejects `body.p_user_id` as `field_not_allowed_p_user_id`), `p_old_password`/`p_new_password` direct body checks (1..200 chars), same-as-old rejection (`password_same_as_old`), RPC call via `supabaseAdmin.rpc('change_password', ...)`, error mapping (RPC `true` → 200, RPC `false` → 400 `invalid_credentials`, RPC error → 500 `change_password_failed`). Added to ALLOWED_ACTIONS Set + dispatcher switch between `staff-cancel-tire-booking` and `default`. **(2) Frontend**: `lib/api/staff-actions.ts` added `changeStaffPassword(oldPassword, newPassword)` wrapper using `dispatchStaffCall`. `components/admin/ChangePasswordWizard.tsx`: removed direct `supabase.rpc('change_password', {p_user_id, ...})` call, replaced with `await changeStaffPassword(oldPassword, newPassword)`. Russian error mapping for `invalid_credentials` / `password_same_as_old` / `field_not_allowed_p_user_id` / `password_required`. **(3) Migration 016** REVOKE EXECUTE FROM PUBLIC + FROM anon + FROM authenticated on `change_password`. 4-step checklist (entry 20d) verified on test DB: proacl = `{postgres=X/postgres, service_role=X/postgres}`, `has_function_privilege('anon',...)` = f, `has_function_privilege('authenticated',...)` = f, `has_function_privilege('service_role',...)` = t, SET ROLE anon + actual call → permission denied. **(4) Test cluster** post-REVOKE: **79/80 PASS** (E1, E2, E4, E5, E6, E7 all PASS; E3 fails on deployed `5d55f5e` because readString throws `p_old_password_required` instead of unified `password_required` — fixed in local commit `3e2a068`, not yet pushed). **Blocker**: `git push origin main` fails with `fatal: could not read Username for 'https://github.com': Device not configured` — macOS keychain credential item for `github.com` is no longer accessible from this shell session (git-credential-osxkeychain returns empty + "Device not configured"). User must re-authenticate GitHub credential on Mac (Keychain Access or `security delete-internet-password -s github.com` + retry). Until then, 3 local commits are preserved (no `git push --force` needed, just regular push when creds work). |
| 20d | **⚠️ GENERAL RULE for all future REVOKE statements** | 📜 Documentation rule — **REVOKE FROM PUBLIC is NOT sufficient** to remove a grant. Each function and table must be REVOKE'd against ALL THREE grantee scopes separately: (1) `FROM PUBLIC` (removes the meta-role grant `{=X/grantor}`), (2) `FROM anon` (removes explicit `anon=X/grantor`), (3) `FROM authenticated` (removes explicit `authenticated=X/grantor`). **Verified twice in this project**: (a) `add_worker_earnings` had BOTH PUBLIC and explicit anon/authenticated grants (migration 015); (b) `add_tire_worker_earnings` had only PUBLIC (migration 012). **Mandatory checklist** after EVERY REVOKE migration: (i) `SELECT proname, proacl::text FROM pg_proc WHERE proname = '<func>'` — must NOT contain `=X/postgres` for anon, authenticated, or PUBLIC; (ii) `SELECT has_function_privilege('anon', '<func>(...)', 'EXECUTE')` and same for authenticated — must return `false`; (iii) `SELECT has_function_privilege('service_role', '<func>(...)', 'EXECUTE')` — must return `true` to preserve dispatcher flow; (iv) `SET ROLE anon; SELECT <func>(...); RESET ROLE` — must fail with `permission denied`. For tables: replace `pg_proc`/`has_function_privilege` with `pg_class`/`relacl::text` and `has_table_privilege`. Apply this checklist to migrations 012/013/014/015 retroactively (they passed smoke tests, but the rule should be standard practice for all future migrations). |
| 20f | **🔒 Phase D — Realtime WS smoke deployed (security-side closed)** | ✅ **Done** — инфра-уровневый WS smoke 4/4 PASS с явным `claims_role` per test. Migration `023_realtime_publication_membership.sql` applied (10 таблиц в `supabase_realtime` publication, idempotent DO block). `lib/_supabase-wrapper.ts` → `setRealtimeAuth(t)` через `client.realtime.setAuth(jwt)` + `lib/realtime-auth.ts` (DI). `App.tsx:1014+` `handleLogout = setSessionToken(null) + removeAllChannels().catch(()=>{})`. **Test cluster** (4/4 PASS, дословные `realtime.subscription` строки): Test 1 admin JWT → `id=310\|role=authenticated\|claims_role=authenticated`, получил полный payload `bookings` (45+ колонок, включая `client_name`, `cancel_comment`); Test 2 anon → `id=311\|role=anon\|claims_role=anon`, получил **только metadata** (`schema, table, eventType, errors=['Error 401: Unauthorized']`), `new:{}, old:{}` — пустые; Test 3 admin + removeAllChannels() → `id=312\|claims_role=authenticated`, pre=1 post=1 (callback не вызван повторно); Test 4 fresh client (re-login модель) → `id=313\|claims_role=authenticated`, новый payload с `cancel_comment='test4 relogin'`. **Root cause Test 2 (фикс найден в реальном времени)**: `client.realtime.setAuth(jwt)` — **async**, обязательно `await` ДО `channel.subscribe()`, иначе `claims_role` в `realtime.subscription` остаётся `anon` (Phase D скрипт исправлен: 3 setAuth call'а теперь awaited). **Script**: `realtime-smoke.mjs` (`.gitignore`d) — Node WS smoke через `@supabase/supabase-js` + `realtime: { transport: ws }` + `await realtime.setAuth(jwt)`. Diagnostic helpers (`realtime-diag.mjs`, `realtime-diag2.mjs`, `get-client-jwt.mjs`) тоже `.gitignore`. Anon key ROTATED (старый iat=1742895817 → новый iat=1787672694, извлечён из bundle `index-*.js`): `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRhbm9ib25ncXpieGlseXZkd2lnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODc2NzI2OTQsImV4cCI6MjEwMzI0ODY5NH0.IY8b5-izkcW2HgLf-N1QyXHGjccVP1NXjUV_I0sThAI`. `browser-smoke-checklist.mjs` обновлён этим ключом, 16/16 PASS. **Cleanup verify**: 0 test rows. **Итог**: RLS data-enforcement работает полностью; `removeAllChannels()` рвёт подписку; re-login восстанавливает. Realtime security-side = **closed**. Vercel build не требовался (только DB + ws-smoke скрипт, не code-deploy). |
| 20g | **⚠️ Known accepted risk — signal-channel leak для anon WS** | 📜 Документированное принятое ограничение (не баг, не bypass). **Что leak**: anon WS-подписчик на `supabase_realtime` publication получает metadata-событие для таблиц, на которые у него нет RLS SELECT grant: `{schema: 'public', table: 'organization_drivers', eventType: 'UPDATE', new: {}, old: {}, errors: ['Error 401: Unauthorized']}`. То есть anon узнаёт **факт изменения** (какая таблица, какой тип события) + видит 401 в `errors`, но **не видит данные строки** (`new:{}` и `old:{}` пустые). **Root cause**: `realtime.apply_rls(wal jsonb)` — Managed Supabase функция (587 строк), которая через `set_config('role', claims_role)` + execute prepared statement делает RLS-enforcement. Когда anon WS subscriber открывает канал, ему доставляется signal-event с заполненными metadata и пустыми `new/old`. Это **стандартное поведение Supabase Realtime**, не bypass нашего кода. **Что НЕ leak**: (1) Конкретные значения колонок (`id`, `full_name`, `phone`, `organization_id` — все пусты); (2) Какие именно поля изменились; (3) Значения до/после. **Mitigation deferred to Phase E** (per-user filter сократит scope подписок — anon не будет открывать канал на таблицы, которые ему заведомо не нужны). **Принято**, потому что data-leak = 0, signal-leak — низкий (теоретический риск оценки активности бизнеса по частоте событий через timing-канал, не data-leak). **Перед каждым REVOKE-шагом Phase 2.5** повторять `realtime-smoke.mjs` — не предполагать по аналогии. |
| 21 | **Phase E (Realtime per-user filter)** | ⏸ **Deferred** — больше **НЕ security-фикс** (RLS уже защищает данные). Теперь это оптимизация из двух независимых частей: **(a) Client-to-client data isolation** (высокий приоритет, НАСТОЯЩИЙ data-leak): таблицы `bookings`, `clients`, `client_cars`, `tire_bookings`, `loyalty_carwash_progress` — текущие `public_all_access USING(true)` политики разрешают client A читать данные client B и через REST, и через Realtime. Realtime smoke это неявно подтвердил: Test 1 admin получил полный `bookings` payload без owner-фильтра, и в текущем state клиент A тоже получит аналогичный payload по чужим bookings через свой JWT. **Требует Category C composite RLS** с `bookings.client_id → clients.profile_id` (или equivalent для каждой таблицы). **(b) Signal-noise reduction via per-user filter** (низкий приоритет): каждый client подписывается только на свои row-id, не на всю таблицу — убирает metadata-шум для ролей, которым чужие события не нужны. Можно делать вместе с (a) или позже. **Перед Phase E** нужен read-only recon по de facto UI-путям, через которые client A сегодня реально видит данные client B (см. active task ниже). |
| 22 | **🔴 Active task — client-to-client data isolation recon** | ⏸ **Read-only, no code changes** — владелец запросил перед началом Phase E. Цель: для каждой из 5 таблиц (`bookings`, `clients`, `client_cars`, `tire_bookings`, `loyalty_carwash_progress`) найти **de facto** UI-пути, через которые client A читает данные client B сегодня. Не «гипотетически» через RLS USING(true), а **через какой конкретный экран/хук/компонент/query chain**. План: (1) grep всех `.from('bookings')`, `.from('clients')` и т.д. в `components/client/`, `features/`, `shared/hooks/`, `App.tsx`, `lib/api/`. (2) Для каждого callsite — какой query, какой фильтр (или его отсутствие), в каком UI-контексте (свои данные или чужие). (3) Для каждой таблицы выписать: «таблица X — client A видит данные client B через путь Y (компонент Z → хук W → query Q)». (4) Отдельно отметить где уже сейчас через dispatcher (`api/client.ts`) — там фильтрация на сервере, и для этих путей data-leak закрыт. (5) Финальный артефакт — таблица в этом entry после recon. Не править код, не писать миграции, не коммитить — только прочитать и зафиксировать находки |
| 22a | **📊 client-to-client data isolation recon — НАХОДКИ** | ⏸ **Read-only result** — таблица de facto data-leak путей по 5 таблицам. Не гипотеза, а grep + read.<br><br>**RLS baseline** (anon+authenticated SELECT на всех 5 = HTTP 200): `public_all_access USING(true)` для всех (PROJECT_STATE.md entry 110). Realtime Test 1 (admin JWT, `claims_role=authenticated`) подтвердил: полный payload bookings доставляется без owner-фильтра. **Значит client JWT с `claims_role=authenticated` тоже получит полный payload**, если subscription filter не работает или не задан.<br><br>### A. REST-side (anon-SELECT через `lib/api/*.ts`)<br><br>\| Функция \| Где вызывается (UI) \| Фильтр \| Data-leak сейчас? \|<br>\|---\|---\|---\|---\|<br>\| `getBookingsByProfileId(profileId)` \| `OnlineBookingWizard.tsx:143`, `useActiveBookings.ts:36` (через `getAllBookingsForClient`), `useBookingHistory.ts:33` (Mini App + MyGarage) \| `.eq('created_by_profile_id', profileId)` \| ✅ НЕТ — filter по создателю, client A не видит чужие bookings (если создатель != A, строка не вернётся). **Но**: bookings созданные admin без `created_by_profile_id` не попадают ни одному клиенту через этот путь. Это data-loss для клиента, не data-leak \|<br>\| `getAllBookingsForClient(profileId, profilePhone)` \| `useActiveBookings.ts:36` (MyGarage) \| personal по `created_by_profile_id` + org-driver по `driver_id` через lookup phone \| ✅ НЕТ (тот же механизм — filter по created_by_profile_id + driver lookup по чужому phone не пройдёт) \|<br>\| `getTireBookingsByProfileId(profileId)` \| `OnlineTireBookingWizard.tsx:149`, `useActiveBookings.ts`, `useBookingHistory.ts` \| `.eq('created_by_profile_id', profileId)` \| ✅ НЕТ \|<br>\| `getAllTireBookingsForClient(profileId, profilePhone)` \| `useActiveBookings.ts:43` (MyGarage) \| personal + driver_id по phone \| ✅ НЕТ \|<br>\| `getBookingsByDate(date)` / `getQuickBookings(date)` / `getCompletedBookingsByDate(date)` / `getTireBookingsByDate(date)` / `getCompletedTireBookingsByDate(date)` \| `App.tsx:585/602/620/710/764`, `SummaryPage.tsx:316-317` \| только по date / status \| n/a — **только staff UI** (App.tsx role-check admin/owner, SummaryPage admin). Эти функции никогда не вызываются из client-side компонентов, client к ним не имеет доступа через Mini App. Но сами по себе они — anon-SELECT всех bookings за дату. **Risk: если в будущем кто-то дёрнет их из client-компонента, data-leak** \|<br>\| dispatcher `get-my-bookings` / `get-tire-bookings` (api/client.ts) \| `ClientBookingWrapper.tsx`, `ClientTireBookingWrapper.tsx` (Mini App timeline) \| Server-side через `clients.profile_id=jwt.profile_id` \| ✅ НЕТ (правильный ownership chain) \|<br>\| `getMyLoyaltyProgressAction` (dispatcher api/client.ts) \| `useLoyaltyProgress.ts:35`, `OnlineBookingWizard.tsx` \| Server-side через `clients.profile_id=jwt.profile_id` \| ✅ НЕТ \|<br><br>**REST-side вывод**: client-to-client data-leak через REST для 5 таблиц **закрыт уже сейчас** (Slice #1 #3a #3b dispatcher path + `created_by_profile_id` filter в legacy API). `getBookingsByDate`/etc. остаются staff-only функциями без client-caller.<br><br>### B. Realtime-side (`supabase.channel().on('postgres_changes', ...)`)<br><br>\| Hook / Component \| Table \| Subscription `filter` \| De facto data-leak? \|<br>\|---\|---\|---\|---\|<br>\| `shared/hooks/useLoyaltyProgress.ts:65` \| `loyalty_carwash_progress` \| ❌ **NO filter** \| 🔴 **CRITICAL** — `payload.new.total_washes_with_body` напрямую в `setCurrentWashes()` (line 75), `payload.new.free_wash_pending` в `setFreeWashPending()` (line 76). Если RLS не блокирует, client A получает payload.new для client B и **применяет в своём state** → функционально-видимый баг: client A видит «у вас бесплатная мойка» + чужой счётчик в своём UI \|<br>\| `shared/hooks/useActiveBookings.ts:76` \| `bookings` \| ✅ `created_by_profile_id=eq.${profileId}` \| ✅ **Закрыто** (после P2 verification) — WS filter narrowing работает server-side, payload содержит только подходящие rows \|<br>\| `shared/hooks/useActiveBookings.ts:119` \| `tire_bookings` \| ✅ filter \| ✅ Закрыто \|<br>\| `shared/hooks/useBookingHistory.ts:73` \| `bookings` \| ✅ filter \| ✅ Закрыто \|<br>\| `shared/hooks/useBookingHistory.ts:112` \| `tire_bookings` \| ✅ filter \| ✅ Закрыто \|<br>\| `shared/hooks/useClientCars.ts:123` \| `client_cars` \| ✅ `client_id=eq.${clientId}` \| ✅ Закрыто \|<br>\| `components/client/ClientBookingWrapper.tsx:180` \| `bookings` \| ❌ **NO filter** \| ⚠️ YES (P1 — фиксим в этом цикле) — handler перезагружает redacted RPC, но `payload.new` с чужими bookings в console.log + JS-memory. Через DevTools атакующий вытащит \|<br>\| `components/client/ClientTireBookingWrapper.tsx:309` \| `tire_bookings` \| ❌ **NO filter** \| ⚠️ YES (P1 — фиксим в этом цикле) — handler перезагружает dispatcher `fetchOwnTireBookings`, но `payload.new` в console.log + memory \|<br><br>### C. Realtime-side для `clients` и `client_cars`<br><br>\| Caller \| Table \| Контекст \|<br>\|---\|---\|---\|<br>\| `App.tsx:867` \| `clients` \| staff dashboard (admin/owner), не client UI \|<br>\| `App.tsx:902` \| `client_cars` \| staff dashboard, не client UI \|<br>\| `useClientCars.ts:123` \| `client_cars` \| covered above (filter есть) \|<br><br>Прямых client-side Realtime-подписок на `clients` нет — client не подписывается на `clients` напрямую. `MyGarage` тянет client-cars через dispatcher + `useClientCars` с filter.<br><br>### D. Сводка по приоритетам для Phase E (a) — client-to-client data isolation<br><br>**🔴 P0 — функциональный data-leak через state**: `useLoyaltyProgress.ts` — client A видит счётчик client B в своём UI. **Один файл, 1 fix.**<br>**🟠 P1 — payload leak в console.log + memory для ВСЕХ клиентских Realtime-подписок**: если кто-то через DevTools читает state/console — получает данные client B. 8 callsites в 7 файлах.<br>**🟢 P2 — защита держится на filter-parameter, не на RLS**: `useActiveBookings`, `useBookingHistory`, `useClientCars` работают потому что filter есть в коде. При рефакторинге или ошибке — сломается. 5 callsites в 3 файлах.<br>**⚪ P3 — REST legacy `getBookingsByDate`/etc.**: остаются staff-only, нет client-caller. Не блокер, но требует фикса или явного запрета на будущее (например, добавить role-check в начало функции или вообще убрать legacy path).<br><br>### E. Что НЕ затронуто этим recon (закрыто раньше)\n- `clients` direct client-side Realtime: нет callsites\n- `clients` REST через dispatcher `get-my-cars` etc.: server-side filter\n- `bookings` create/update/delete через dispatcher (Slice #1, #3a, #3b): server-side ownership check\n- `loyalty_carwash_progress` reads через dispatcher `getMyLoyaltyProgressAction`/`getMyWashesUntilNextFreeWashAction`: server-side filter\n- `client_cars` create/update/delete через dispatcher: server-side check\n- `tire_bookings` create/cancel через dispatcher: server-side ownership check (Slice #2)\n\n### F. Что НЕ покрыто этим recon (требует дальнейшей работы перед Phase E (a))\n- **Live verification**: нужно подтвердить что Supabase WS-`filter` parameter реально narrowing payload при `claims_role=authenticated` + RLS USING(true). Smoke-тест: client JWT subscribe на `bookings` с filter `created_by_profile_id=eq.X` vs без filter — сравнить payload. Это даст ответ «Conditional YES = YES или NO».\n- **`getBookingsByDate` etc. role guard**: добавить throw если caller не staff (defense-in-depth, на случай если в будущем client-компонент случайно дёрнет).\n- **Покрытие тестами**: после фикса P0 нужны автотесты на «client A subscribe → payload events с client B booking НЕ приходят». Это можно сделать через новый smoke-script по аналогии с `realtime-smoke.mjs`.\n<br>**Следующий шаг после recon**: согласовать с владельцем приоритеты P0/P1/P2/P3 и порядок фиксов. **Не начинать код без явного ОК.** |
| 22b | **🟢 P2 live verification — filter narrowing работает server-side** | ✅ **Done** — `realtime-p2-smoke.mjs` (в проекте). 5 сценариев через client JWT:<br>• C1 admin no filter (CONTROL, Phase D Test 1 mirror) → YES ✅<br>• C2 anon no filter (CONTROL, Phase D Test 2 mirror) → YES (полный payload 45 колонок для `bookings`) ✅⚠️ — см. entry 22c<br>• T1 client no filter на `bookings` → YES (полный payload 45 колонок для foreign booking) — подтверждение P1<br>• T2 client filter `created_by_profile_id=A` на `bookings`, UPDATE foreign → **NO EVENT** (server не отправляет)<br>• T3 client filter `client_id=A` на `bookings`, UPDATE foreign → **NO EVENT** (server не отправляет)<br><br>**Вывод**: Supabase WS `filter` parameter действительно narrowing server-side для `claims_role=authenticated` (нет bypass через metadata-канал). **P2 закрывается без кода**: 5 callsites (`useActiveBookings.ts:76/119`, `useBookingHistory.ts:73/112`, `useClientCars.ts:123`) уже имеют filter — server отправляет event только если row подходит под filter, иначе 0 событий. Никаких payload в JS memory. Никаких console.log с чужими данными.<br><br>**Замечание для будущего (не security-fix)**: `useActiveBookings` / `useBookingHistory` используют `created_by_profile_id` filter, не `client_id`. Это работает для self-created bookings, но не для admin-created bookings, где `created_by_profile_id=admin`. Это data-loss для клиента (он не получит уведомление), не data-leak. Возможное улучшение: перейти на `client_id=eq.${ownClientId}` filter для правильной ownership-семантики — отдельная задача.<br><br>**НЕ фиксим P2 в текущем цикле.** |
| 22c | **🆕 NEW FINDING: anon получает full Realtime payload на `bookings` с USING(true)** | ⚠️ **Открыто в P2 smoke** — контрол C2 anon без setAuth, без filter, UPDATE на `bookings` → event received с **полным payload 45 колонок** (`client_name`, `phone`, `car_model`, `plate_number`, `services`, `payment_method`, `signature_data`), `errors=null` (не `['Error 401']`).<br><br>**Причина**: Phase D Test 2 на `organization_drivers` показал metadata-only с `errors=['401']` — потому что `organization_drivers` имеет узкие RLS policies после Slice #3d. `bookings` / `tire_bookings` / `client_cars` / `clients` имеют `public_all_access USING(true)` для anon, поэтому RLS pass для anon → full data delivery через Realtime.<br><br>**Что это означает**: (1) anon через Realtime читает ВСЕ bookings/tire_bookings/client_cars/clients с полными PII — это **REST-equivalent data-leak через WS**, который раньше не проверялся. (2) UI это не показывает — anon в Mini App открывает dispatcher path, dispatcher фильтрует на server. Но **через DevTools / прямой WS subscribe** anon получает все данные. (3) Это **не Phase E problem** — это **Phase 2.5 anon REVOKE problem**, относится к `realtime` GRANTs/privileges, не к client-side filter.<br><br>**Действие**: добавить отдельный task — **Phase 2.5b: REVOKE anon access к Realtime на tables с PII** (5 таблиц). Перед фиксом — повторить `realtime-p2-smoke.mjs` с C2 на каждой из 5 целевых таблиц, чтобы подтвердить масштаб.<br><br>**Не блокирует Phase E (a)** — разные категории: Phase E (a) — client-to-client (с authenticated JWT), Phase 2.5b — anon-to-any (без JWT). |
| 22d | **📋 P3 deferred (tech-debt)** | ⏸ **Tech-debt item** — defensive hardening для `getBookingsByDate` / `getQuickBookings` / `getCompletedBookingsByDate` / `getTireBookingsByDate` / `getCompletedTireBookingsByDate` (anon-SELECT всех bookings/tire_bookings за date без profile filter). Сейчас нет client-caller (используются только staff UI: `App.tsx`, `SummaryPage.tsx`). Гипотетический риск: если кто-то в будущем дёрнет из client-компонента, data-leak. **Не в этом цикле**. Действие когда дойдёт: добавить throw если `userRole !== 'admin' && userRole !== 'owner'` в начало каждой функции, или удалить legacy path после полного port на dispatcher. |<br>\| `getTireBookingsByProfileId(profileId)` \| `OnlineTireBookingWizard.tsx:149`, `useActiveBookings.ts`, `useBookingHistory.ts` \| `.eq('created_by_profile_id', profileId)` \| ✅ НЕТ \|<br>\| `getAllTireBookingsForClient(profileId, profilePhone)` \| `useActiveBookings.ts:43` (MyGarage) \| personal + driver_id по phone \| ✅ НЕТ \|<br>\| `getBookingsByDate(date)` / `getQuickBookings(date)` / `getCompletedBookingsByDate(date)` / `getTireBookingsByDate(date)` / `getCompletedTireBookingsByDate(date)` \| `App.tsx:585/602/620/710/764`, `SummaryPage.tsx:316-317` \| только по date / status \| n/a — **только staff UI** (App.tsx role-check admin/owner, SummaryPage admin). Эти функции никогда не вызываются из client-side компонентов, client к ним не имеет доступа через Mini App. Но сами по себе они — anon-SELECT всех bookings за дату. **Risk: если в будущем кто-то дёрнет их из client-компонента, data-leak** \|<br>\| dispatcher `get-my-bookings` / `get-tire-bookings` (api/client.ts) \| `ClientBookingWrapper.tsx`, `ClientTireBookingWrapper.tsx` (Mini App timeline) \| Server-side через `clients.profile_id=jwt.profile_id` \| ✅ НЕТ (правильный ownership chain) \|<br>\| `getMyLoyaltyProgressAction` (dispatcher api/client.ts) \| `useLoyaltyProgress.ts:35`, `OnlineBookingWizard.tsx` \| Server-side через `clients.profile_id=jwt.profile_id` \| ✅ НЕТ \|<br><br>**REST-side вывод**: client-to-client data-leak через REST для 5 таблиц **закрыт уже сейчас** (Slice #1 #3a #3b dispatcher path + `created_by_profile_id` filter в legacy API). `getBookingsByDate`/etc. остаются staff-only функциями без client-caller.<br><br>### B. Realtime-side (`supabase.channel().on('postgres_changes', ...)`)<br><br>\| Hook / Component \| Table \| Subscription `filter` \| De facto data-leak? \|<br>\|---\|---\|---\|---\|<br>\| `shared/hooks/useLoyaltyProgress.ts:65` \| `loyalty_carwash_progress` \| ❌ **NO filter** \| 🔴 **CRITICAL** — `payload.new.total_washes_with_body` напрямую в `setCurrentWashes()` (line 75), `payload.new.free_wash_pending` в `setFreeWashPending()` (line 76). Если RLS не блокирует, client A получает payload.new для client B и **применяет в своём state** → функционально-видимый баг: client A видит «у вас бесплатная мойка» + чужой счётчик в своём UI \|<br>\| `shared/hooks/useActiveBookings.ts:76` \| `bookings` \| ✅ `created_by_profile_id=eq.${profileId}` \| ⚠️ **Conditional YES** — WS filter narrowing на стороне Supabase работает **только если** RLS разрешает все rows. Поскольку RLS USING(true), Supabase применит client-supplied filter на уровне WS и **payload.new содержит только `created_by_profile_id=A`**. НО если кто-то уберёт filter — сломается. Также handler (line 82) делает optimistic `prev.map(b => b.id === payload.new.id ? payload.new : b)` — payload.new для своего row **напрямую в state** (это ОК, это свои данные). **Защита держится на filter-parameter**, не на RLS \|<br>\| `shared/hooks/useActiveBookings.ts:119` \| `tire_bookings` \| ✅ filter \| ⚠️ Conditional YES (same) \|<br>\| `shared/hooks/useBookingHistory.ts:73` \| `bookings` \| ✅ filter \| ⚠️ Conditional YES (same) \|<br>\| `shared/hooks/useBookingHistory.ts:112` \| `tire_bookings` \| ✅ filter \| ⚠️ Conditional YES (same) \|<br>\| `shared/hooks/useClientCars.ts:123` \| `client_cars` \| ✅ `client_id=eq.${clientId}` \| ⚠️ Conditional YES (same) \|<br>\| `components/client/ClientBookingWrapper.tsx:180` \| `bookings` \| ❌ **NO filter** \| ⚠️ YES (но mitigated) — `loadOccupancyForDate(bookingDate)` перезагружает из `rpc/get_public_booking_slots` (redacted форма: `client_name='Занято'`, пустые `car_model/plate_number/phone/services`). **UI показывает только redacted**, но `payload.new` для всех bookings временно в console.log + JS-memory клиента. Через DevTools атакующий с доступом к машине клиента может вытащить \|<br>\| `components/client/ClientTireBookingWrapper.tsx:309` \| `tire_bookings` \| ❌ **NO filter** \| ⚠️ YES (mitigated) — handler перезагружает через dispatcher `fetchOwnTireBookings` (server-side filter). UI показывает только свои. Но `payload.new` для чужих tire_bookings в console.log + memory \|<br><br>### C. Realtime-side для `clients` и `client_cars`<br><br>\| Caller \| Table \| Контекст \|<br>\|---\|---\|---\|<br>\| `App.tsx:867` \| `clients` \| staff dashboard (admin/owner), не client UI \|<br>\| `App.tsx:902` \| `client_cars` \| staff dashboard, не client UI \|<br>\| `useClientCars.ts:123` \| `client_cars` \| covered above (filter есть) \|<br><br>Прямых client-side Realtime-подписок на `clients` нет — client не подписывается на `clients` напрямую. `MyGarage` тянет client-cars через dispatcher + `useClientCars` с filter.<br><br>### D. Сводка по приоритетам для Phase E (a) — client-to-client data isolation<br><br>**🔴 P0 — функциональный data-leak через state**: `useLoyaltyProgress.ts` — client A видит счётчик client B в своём UI. **Один файл, 1 fix.**<br>**🟠 P1 — payload leak в console.log + memory для ВСЕХ клиентских Realtime-подписок**: если кто-то через DevTools читает state/console — получает данные client B. 8 callsites в 7 файлах.<br>**🟢 P2 — защита держится на filter-parameter, не на RLS**: `useActiveBookings`, `useBookingHistory`, `useClientCars` работают потому что filter есть в коде. При рефакторинге или ошибке — сломается. 5 callsites в 3 файлах.<br>**⚪ P3 — REST legacy `getBookingsByDate`/etc.**: остаются staff-only, нет client-caller. Не блокер, но требует фикса или явного запрета на будущее (например, добавить role-check в начало функции или вообще убрать legacy path).<br><br>### E. Что НЕ затронуто этим recon (закрыто раньше)\n- `clients` direct client-side Realtime: нет callsites\n- `clients` REST через dispatcher `get-my-cars` etc.: server-side filter\n- `bookings` create/update/delete через dispatcher (Slice #1, #3a, #3b): server-side ownership check\n- `loyalty_carwash_progress` reads через dispatcher `getMyLoyaltyProgressAction`/`getMyWashesUntilNextFreeWashAction`: server-side filter\n- `client_cars` create/update/delete через dispatcher: server-side check\n- `tire_bookings` create/cancel через dispatcher: server-side ownership check (Slice #2)\n\n### F. Что НЕ покрыто этим recon (требует дальнейшей работы перед Phase E (a))\n- **Live verification**: нужно подтвердить что Supabase WS-`filter` parameter реально narrowing payload при `claims_role=authenticated` + RLS USING(true). Smoke-тест: client JWT subscribe на `bookings` с filter `created_by_profile_id=eq.X` vs без filter — сравнить payload. Это даст ответ «Conditional YES = YES или NO».\n- **`getBookingsByDate` etc. role guard**: добавить throw если caller не staff (defense-in-depth, на случай если в будущем client-компонент случайно дёрнет).\n- **Покрытие тестами**: после фикса P0 нужны автотесты на «client A subscribe → payload events с client B booking НЕ приходят». Это можно сделать через новый smoke-script по аналогии с `realtime-smoke.mjs`.\n<br>**Следующий шаг после recon**: согласовать с владельцем приоритеты P0/P1/P2/P3 и порядок фиксов. **Не начинать код без явного ОК.** |
| 22e | **🔴 Phase 2.5b — REVOKE anon WS access на PII-таблицах (HIGH PRIORITY, выше P3)** | 🚨 **TODO, не откладывать.** Live PII exposure risk: anon (без JWT, без setAuth) подписывается через Realtime на `bookings` / `tire_bookings` / `client_cars` / `clients` / `loyalty_carwash_progress` и получает **полный payload всех строк с PII** (45 колонок: `client_name`, `phone`, `car_model`, `plate_number`, `services`, `payment_method`, `signature_data`, `created_by_profile_id`, etc.). Причина — `public_all_access USING(true)` + `supabase_realtime` publication, которая публикует эти таблицы. UI anon не показывает (Mini App dispatcher фильтрует), но **через DevTools / прямой WS subscribe любой анонимный пользователь читает всю PII клиентов и броней в реальном времени без авторизации**. Это серьёзнее чем Phase E (a) косметика.<br><br>**Скоуп** (5 таблиц): `bookings`, `tire_bookings`, `client_cars`, `clients`, `loyalty_carwash_progress`. Не входит `organization_cars` / `organization_drivers` — там RLS уже узкий после Slice #3d, anon получает только metadata (Phase D Test 2).<br><br>**Перед фиксом**: прогнать `realtime-p2-smoke.mjs` с control C2 на каждой из 5 таблиц отдельно, чтобы подтвердить масштаб (одна таблица уже подтверждена — `bookings`, остальные 4 нуждаются в verify).<br><br>**Возможные механизмы** (нужно recon перед фиксом):<br>1. **Migration REVOKE** на `realtime` schema: `REVOKE USAGE ON SCHEMA realtime FROM anon` или per-table `REVOKE SELECT ON realtime.messages FROM anon` — Supabase не даёт редактировать `realtime.messages` напрямую (Managed), но можно через publication.<br>2. **Per-table publication toggle**: `ALTER PUBLICATION supabase_realtime DROP TABLE public.bookings` — но это сломает WS для admin/owner, которые должны получать events. Решение — split-publication: создать `supabase_realtime_staff` для admin/owner, переключить `supabase_realtime` только на non-PII таблицы.<br>3. **Composite RLS on realtime**: добавить `auth.role() = 'authenticated'` или `auth.jwt()->>'app_role' IN ('admin','owner','client')` в policy — но для anon (no JWT) это должно привести к ошибке, не к bypass.<br><br>**Ожидаемый результат фикса**: anon подписка на эти 5 таблиц → event с `errors=['401 Unauthorized']` (metadata-only signal, без `new`/`old`), как Phase D Test 2 на `organization_drivers`. Admin/owner/client (с JWT) продолжают получать events нормально.<br><br>**Приоритет**: выше P3, в очереди сразу после текущего Phase E (a) deploy подтверждения. Не убирать в фон, не откладывать на «когда-нибудь». Владелец явно попросил не потерять (см. message 2026-08-31). |

| 24 | **🔴 HOTFIX — workers UPDATE 403 на repeat shift start (блокировал операционную работу)** | ✅ **Done** — commit `3ed5afc` deployed. Root cause: `lib/api/workers.ts:482 startWorkerShift` имел legacy fallback-ветку `if (worker.base_rate_taken_today) → supabase.from('workers').update({is_working_today: true, status: 'available'})` через anon-key. Migration 020 (Slice #3d, commit `9b8a44f`) REVOKEd INSERT/UPDATE/DELETE на `workers` от authenticated, оставив только SELECT. Fallback-ветка не была портирована на dispatcher при Slice #3d (та же проблема что была с `toggleBox` до fix). Результат — каждый повторный клик «вывести на смену» для worker с `base_rate_taken_today=true` возвращал HTTP 403 permission denied. Мойщик не мог продолжить работу → заказ физически невозможно закрыть.<br><br>**Fix**: `lib/api/workers.ts:482` — удалена legacy fallback-ветка + unused `formatDate()`. Все non-early-return paths идут через `startStaffWorkerShift(workerId)` dispatcher (api/staff.ts:2250, service_role + `start_worker_shift` RPC). Pre-check `getWorkerById` сохранён для UX (быстрый early-return без round-trip если `is_working_today=true` уже).<br><br>**RPC idempotency (dump:1718-1766)**: `start_worker_shift` уже содержит `IF v_worker.base_rate_taken_today THEN RETURN v_worker` (no-op, без UPDATE) — поэтому fallback-ветка была дублирующей логикой, не несущей уникальной семантики. RPC корректно обрабатывает оба случая: (a) `base_rate=false` → UPDATE + INSERT work_shifts; (b) `base_rate=true && is_working=true` → noop (idempotent).<br><br>**Path A (минимальный fix) обоснован live data-check** (`recon-scenario3-v2.mjs`): сценарий `base_rate_taken_today=true AND is_working_today=false` имеет **0 current instances** (9 workers, все idle, last_shift=null) и **0 historical occurrences** за 30 дней (21 work_shifts row, все `status=finished` с earnings > 0, уникальные по `(worker_id, work_date)`). Fallback-ветка была мёртвым кодом с момента Slice #3d.<br><br>**Verification**: live-smoke через dispatcher (api/staff?action=…) на deployed Vercel, 4/4 PASS: (1) fresh worker → 200 `work=true base=false`, (2) repeat worker (base=true) → **200 (был 403)** — RPC idempotent noop, (3) fresh tire_worker → 200 `work=true`, (4) fresh admin → 200 `work=true last_shift=2026-08-31`. Browser-smoke regression: 16/16 PASS.<br><br>**No DB migration, no API changes**. Vercel function count 12/12 (unchanged). Diff: -40 lines, +18 lines (комментарии). |
| 24a | **📋 Tech-debt — start_worker_shift RPC idempotency при `base_rate_taken_today=true && !is_working_today`** | ⏸ **Deferred until scenario 3 is real production data.** Path B (RPC change) отложен: если когда-либо встретится `base_rate_taken_today=true && is_working_today=false` (worker взял base_rate, завершил работу, status → idle), текущий RPC вернёт без UPDATE → `is_working_today` останется `false` несмотря на повторный клик «вывести на смену». Для покрытия нужен RPC update (см. plan Phase B в моём сообщении перед fix). **Мониторинг**: запрос `SELECT id, full_name FROM workers WHERE base_rate_taken_today = true AND is_working_today = false` — если возвращает > 0 строк, пора применять Path B. До тех пор Path A (dispatcher with idempotent noop) достаточен. |
---
...
| Slice #3c — RLS Category A — staff JWT работает в PostgREST | ⚠️ **КРИТИЧНО: подтверждено экспериментально** (см. entry 20e-эксперимент). Когда admin логинится через `/api/login`, наш `/api/login` генерирует HS256 JWT с `app_role='admin'` (плюс `role='authenticated'`, `sub=profile_id`). Этот JWT передаётся в `Authorization: Bearer <jwt>` через wrappedFetch в `lib/_supabase-wrapper.ts`. PostgREST ВАЛИДИРУЕТ этот JWT (подписан тем же `SUPABASE_JWT_SECRET`, что и goauth) и в `auth.jwt()` выставляет ВСЕ наши кастомные claims включая `app_role`. **Подтверждено через прямой psql + PostgREST тест**: anon (no JWT) → `auth.jwt()` returns `{"role":"anon"}`; admin с JWT → `auth.jwt()` returns `{role:'authenticated', app_role:'admin', sub:'<profile_id>'}`. **Импликация для архитектуры Slice #3c**: Path B (RLS-split admin-SELECT / owner-ALL) **технически валиден** — `auth.jwt()->>'app_role' IN ('admin','owner')` корректно фильтрует direct `.from()` вызовы из admin dashboard без необходимости port'а всех reads на dispatcher. До этого эксперимента моя оценка Slice #3b (и Phase 2 RLS plan в исходном recon) была неправильной — я предполагал что dispatcher-port нужен для reads. Фактически: dispatcher нужен только для WRITES (Phase 2.5/2.6/2.7/Slice #3c writes); reads могут идти через RLS-based filtering. |
| 21 | **Phase 2.4 / Slice #3c (Category A writes) deployed** | ✅ **Done** — 15 dispatcher actions registered in `api/staff.ts` + 15 typed wrappers in `lib/api/staff-actions.ts`. 14 frontend callsites ported (Admins.tsx ×8, SalarySettingsWizard ×1, CompanySettingsWizard ×2, manageBalance.ts ×5). `finishAdminShift` JS wrapper DELETED (dead code, 0 external callers). Migration `017_category_a_rls_split_owner_all_admin_select.sql` applied to test DB (4 sections: grants + drop policies + new RLS + REVOKE EXECUTE on `start_admin_shift`). Migration `018_alter_admins_profile_id_nullable.sql` applied (profile_id NULLable for dispatcher-created internal admin records before profile link). **Authorization matrix** (verified): 4 admin-or-owner (#4, #9, #10, #12) + 11 owner-only. **Test cluster** (3 factual numbers): (а) baseline **80 PASS / 0 FAIL**; (б) Slice-3c admin-rejection tests **+31 PASS** (E1+E3 for all 15 actions); (в) Slice-3c OWNER-PATH happy-path tests **+21 PASS** (E4+E5 for all 11 owner-only actions, where test path seeded earned_today via psql for advance test and tested both idempotent+normal paths for transfer). Final TOTAL: **133 PASS / 0 FAIL**. **8 dispatcher bugs caught by happy-path tests** (NOT by admin-rejection tests): 1) `balance_after` NOT NULL — fixed in 3 commits (`7bfeb34`, `861d673`, `449d206`); 2) `admins.profile_id` NOT NULL with FK — fixed by migration 018; 3) update-salary-settings missing WHERE clause — fixed; 4) admin-transaction actions missing `worker_name` — fixed; 5) adminTransferBalance `earned=0` returned 400 — fixed to idempotent; 6) test typo `full_name_name` vs `full_legal_name`. **Browser smoke** (owner JWT): 4 scenarios all PASS — create-admin (privilege escalation), update-salary-settings (0.41→0.42 then restore), admin-payout-salary (balance 200→150 + ledger row amount=50), delete-salary-transaction (created disposable tx, deleted, rows=0). **Cleanup verify**: 0 test rows, 0 smoke rows, 0 recent salary_transactions, salary_settings restored to 0.4. **Critical**: `finish_admin_shift` RPC does NOT exist in pg_proc (verified), so no REVOKE needed. Production deployment remains gated (separate explicit question). |
| 21a | **Slice #3c follow-up TODO #1**: `admin-payout-salary` E4/E5 SKIP path в автоматическом тесте | ⏸ **Tech debt** — в `test-staff-booking-endpoints.mjs:1434-1446` условие `if (balanceBeforePayout >= 1)` skip'ит тест если у admin на момент preflight `current_balance < 1`. На последнем прогоне balance был 0 (от предыдущего cleanup), тест skip. Browser smoke 4 (в entry 21) подтвердил happy-path end-to-end с `UPDATE … SET current_balance=200` preflight + balance 200→150 + ledger row 50. **Fix (будущий рефакторинг, не сейчас)**: добавить в preflight секцию теста обязательный `execSync('UPDATE … SET current_balance = 100 WHERE id = ${adminIdForTests}')` через `psql`, тогда SKIP не сработает. Не блокер — owner-path happy-path проверен ручным smoke с тем же dispatcher кодом. **Зафиксировано**, чтобы при будущих рефакторингах не потерять — этот action должен быть покрыт автоматическим regression-тестом, не только ручным. |
| 21b | **Slice #3c follow-up TODO #2 — admin profile link invariant** | ⏸ **Known gap, intentional** — после `create-admin` dispatcher (`api/staff.ts:create-admin`) новый admin row существует с `profile_id=NULL` (migration 018 это разрешает). **Что это означает для login**: `/api/login` использует `verify_password(p_login, p_password)` RPC, который ищет по `profiles.login` + `profiles.password_hash` и возвращает `profile.role`. Этот профиль должен иметь `role IN ('admin','owner')`. Если `profile_id` admin row ни на что не link'нут, у этого admin'а **нет auth credentials** — он физически не может залогиниться. **Существующий механизм для клиентов** (`/api/link-client-profile.ts`, Phase 1.5) **НЕ подходит** — он строго проверяет `claims.app_role === 'client'` (admin/owner → 403). **Аналога для staff нет в коде**. Legacy `createAdmin` в `lib/api/admins.ts:77-123` создаёт profile row без `login`/`password_hash` полей, поэтому даже legacy путь НЕ создаёт рабочие credentials. **Текущий onboarding нового admin — manual**: (1) owner создаёт admin row через `create-admin`; (2) владелец БД вручную создаёт profiles row с role='admin', login=…, password_hash=`crypt('…', gen_salt('bf'))`; (3) вручную `UPDATE admins SET profile_id = … WHERE id = …`. **Это явный two-step flow, не баг dispatcher'а** — dispatcher правильно отделяет "создать staff record" от "создать auth profile". Если в будущем понадобится UI wizard для полного onboarding (по аналогии с Wizard для клиентов), это будет отдельный slice. **Зафиксировано как invariant**, чтобы следующий разработчик не решил что `profile_id IS NULL` в новой admin row — это баг. |
| 21c | **Git push workaround для macOS keychain hangs** | ⚙️ **Environment invariant** — на этой машине `git-credential-osxkeychain` иногда возвращает пустую строку при попытке GitHub auth, что приводит к видимому зависанию `git push`. **Решение** (сохранено в этой сессии, работает для всех 5 Slice #3c коммитов): `GIT_TERMINAL_PROMPT=0 GIT_HTTP_LOW_SPEED_LIMIT=200 GIT_HTTP_LOW_SPEED_TIME=10 git push origin main`. Объяснение: `GIT_TERMINAL_PROMPT=0` запрещает prompt на пароль (если keychain пустой, push упадёт fast, не зависнет); low speed limits ускоряют fallback если идёт через сеть. Все будущие push'ы в этой среде — с этим префиксом. |
| 22 | **🔴 PROD BREACH VECTORS — ОБНАРУЖЕНЫ 27.08.2026, РЕШЕНИЕ ВЛАДЕЛЬЦА: ОТЛОЖЕНО ДО ЗАВЕРШЕНИЯ ПОЛНОЙ АРХИТЕКТУРЫ БЕЗОПАСНОСТИ** | � **8 live breach vectors на проде** (avajtwihzjfpytimfbaw, read-only verified): (1) anon SELECT password_hash из profiles — bcrypt хэши паролей сотрудников утекают; (2) anon SELECT card_number/payment_phone из admins/workers/tire_workers — банковские карты и СБП-телефоны утекают; (3) anon SELECT из view tire_bookings_timeline — **809 реальных строк** шиномонтажа с total_price, worker_id, is_paid, booking_date, start_time; (4) anon EXECUTE add_tire_worker_earnings (INVOKER + open salary_transactions RLS) — может создавать фиктивные зарплатные транзакции; (5) anon EXECUTE start_worker_shift; (6) anon EXECUTE start_tire_worker_shift; (7) anon EXECUTE get_next_document_number (SECURITY DEFINER — runs as postgres); (8) anon EXECUTE inventory_usage/restock/arrival/add_inventory_category/delete_inventory_category (5× SECURITY DEFINER — полный bypass RLS на inventory-таблицах). **Demo/prod drift**: migration 012 (REVOKE `add_tire_worker_earnings`) **НЕ применена на проде** — на test DB `{postgres=X, service_role=X}` (REVOKEd), на проде `{=X, anon=X, authenticated=X, service_role=X}` (PUBLIC+anon+auth доступ); `get_next_document_number` имеет **4 overloads на проде vs 2 на test**; **35 RPC на проде vs 47 на test** — 12 функций отсутствуют на проде. **Решение владельца (27.08.2026)**: ничего не фиксим на проде до полной готовности Slice #3d + #3e + Phase 2.5 + Phase 3/3.5 на demo. На прод — **только read-only recon**, никаких REVOKE/GRANT/ALTER/CREATE POLICY/DROP/migration/deploy. Когда вся архитектура готова и провалидирована на demo — переносится на прод одним скоординированным циклом, не разрозненными точечными правками. **Влияние на design migration 021**: per-overload REVOKE, проверка существования функции перед REVOKE (не предполагать что то, что есть на demo, обязательно есть на prod). |
| 23 | **🔒 Slice #3d (Phase 2 RLS Category B) DEPLOYED ON DEMO** | ✅ **Done** — full Slice #3d closed (Step 0 + Step 1: 019 → 020 → 021). **Step 0**: 9 dispatcher proxy actions in `api/staff.ts` (start-worker-shift, start-tire-worker-shift, stop-tire-worker-shift, add-tire-worker-earnings, inventory-usage, inventory-restock, add-inventory-category, delete-inventory-category, inventory-arrival, get-next-document-number) + 9 typed wrappers in `lib/api/staff-actions.ts` + 10 frontend callsite ports в 5 lib/api файлах + TireTechnicianCard.tsx ON+OFF оба через dispatcher. **Migration 019a** (fix for `start_tire_worker_shift` carwash-copy bug): 3-param signature kept, base_rate_taken_today reference removed; new `stop_tire_worker_shift(p_worker_id)` RPC created (1-param, last_shift_date preserved, atomic OFF, FOR UPDATE on active work_shift row only). **Step 1 migrations on demo**: `019_category_anomaly_prep.sql` (A1 view REVOKE + A2 closed_boxes split + A3 tire_service_days split + A6 password_hash column REVOKE + A7 card_number/payment_phone column REVOKE); `020_category_b_path_b_rls.sql` (Path B for 15 Category B staff-only tables + profiles staff SELECT — bookings/tire_bookings deferred to Slice #3e for composite Category B+C RLS); `021_revoke_staff_direct_rpc_execute.sql` (REVOKE EXECUTE on 11 staff-direct RPC overloads, conditional on pg_proc existence for prod-portability). **Regression**: 170/170 PASS after each migration (baseline + 19 Step 0 asserts). **Browser smoke**: dispatcher ON+OFF works, get-next-document-number works, tire shift cycle works through dispatcher post-021-revoke. **4-step §20d verify per object**: proacl, has_*_privilege, SET ROLE anon + actual call → permission_denied ✓. **Prod UNCHANGED** (per entry 22). Vercel function count: 12/12 (no new serverless files). |
