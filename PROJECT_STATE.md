...
| 38 | **Фаза 2 — Slice #3a: Tests E0-E14 — 19 PASS / 0 FAIL на первом deploy** — пред-fligh preflight 5 таблиц + §5.9 rule дали 0 bugs в первом run (cf. Slice #2 имел 4 contract-bugs на старте). `test-staff-endpoints.mjs` E0-E14 + cleanup через `psql` shell-out (`client_cars BEFORE clients` FK order). Покрывает: anonymous-no-token (401), client-JWT-with-staff (403 wrong_role), unknown-action (404), search fields allow-list, create-client valid + collision, create-client-car FK missing + valid, update-client patch + unblock, create-organization + collision, create-org-driver + create-org-car + signature update | Commit fc31d78, deploy |
...
| 15 | **Фаза 2 — Slice #3a (staff client/car/org writes)** | ✅ **Done** — `api/staff.ts` (13 actions dispatcher, 12/12 Vercel Hobby = exactly at limit), `api/_lib/require-staff.ts`, 19/19 E2E PASS on prod alias (`test-staff-endpoints.mjs`). `BookingWizard.tsx` switched (12 anon calls → `dispatchStaffCall`). Pre-flight §5.9 cross-check на 5 целевых таблиц выявил 0 GENERATED columns и поймал 0 багов на первом deploy (vs 4 багов в Slice #2) |
