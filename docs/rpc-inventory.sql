...
BEGIN
  ...
  -- Обновляем только current_quantity (base_quantity НЕ трогаем!)
  UPDATE inventory_items
  SET 
  ...
  INSERT INTO inventory_operations (
    ...
    v_current_qty, p_quantity - v_current_qty, p_quantity,
    v_base_qty, v_base_qty, -- base не меняется
    COALESCE(p_notes, 'Пересчёт остатков'),
    p_created_by
  );
