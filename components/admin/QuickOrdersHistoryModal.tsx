...
export interface QuickOrdersHistoryModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const QuickOrdersHistoryModal: React.FC<QuickOrdersHistoryModalProps> = ({
  isOpen,
  onClose,
  quickBookings,
  selectedDate,
  ...
  // Фильтруем быстрые заказы по статусу и дате
  const filteredBookings = quickBookings.filter(booking => {
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto scroll-mobile">
        <DialogHeader>
          <div className="flex items-center gap-2">
            <DialogTitle className="text-lg md:text-xl font-bold">История быстрых заказов</DialogTitle>
                ...
                <SelectContent>
                  <SelectItem value="done">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="ml-2 text-xs">
                        {quickBookings.filter(b => b.status === 'ГОТОВО' && (!selectedDate || b.booking_date === selectedDate)).length}
                      </Badge>
                    </div>
                  </SelectItem>
                  <SelectItem value="cancelled">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="ml-2 text-xs">
                        {quickBookings.filter(b => b.status === 'ОТМЕНЕНО' && (!selectedDate || b.booking_date === selectedDate)).length}
                      </Badge>
                    </div>
                  </SelectItem>
                </SelectContent>
        ...
        <div className="mt-4">
          {sortedBookings.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              {selectedTab === 'done' ? 'Нет выполненных быстрых заказов' : 'Нет отмененных быстрых заказов'}
            </div>
          ) : (
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-sm">{booking.start_time || 'Быстрый'}</span>
              {booking.end_time && (
                <span className="text-gray-400">→</span>
              )}
              {booking.end_time && (
                <span className="font-semibold text-sm">{booking.end_time}</span>
              )}
            </div>
            <Badge
              variant="outline"
