...
/**
 * Добавить заработок мойщику за завершенный заказ (ТОЛЬКО процент, БЕЗ базовой ставки)
 * Использует RPC функцию с блокировкой FOR UPDATE для защиты от дублей
 * @param workerId - UUID мойщика
 * @param bookingId - UUID заказа
 * @param orderPrice - стоимость заказа
 * @param partnerName - имя партнера (только для режима pair)
 * @returns Обновленный мойщик
 * @throws Error если запрос к базе данных не удался
 */
export async function addWorkerEarningsForBooking(
  ...
  if (!worker.working_mode) {
    throw new Error(`Мойщик ${worker.full_name} не имеет установленного режима работы`);
  }
  ...
  // ✅ Рассчитываем полную цену БЕЗ скидки
  // ⚠️ Поля из БД возвращаются как строки, нужно преобразовать в числа!
  const priceForSalary = parseFloat(booking.price) + (parseFloat(booking.discount) || 0);

  const earnings = worker.working_mode === 'solo'
    ? priceForSalary * settings.worker_solo_commission      // ✅ От полной цены!
    : priceForSalary * settings.worker_pair_commission;     // ✅ От полной цены!

  const cars = worker.working_mode === 'pair' ? 0.5 : 1;

  // ✅ ВЫЗОВ RPC ФУНКЦИИ с блокировкой FOR UPDATE
  const { data, error } = await supabase.rpc('add_worker_earnings', {
    p_worker_id: workerId,
  ...
  if (error) {
    console.error('[Workers] Ошибка при добавлении заработка:', error);
    throw new Error(`Не удалось добавить заработок: ${error.message}`);
  }

  // ✅ Проверка: уже было начислено?
  if (!data.success) {
    console.log(`[Workers] Заказ ${bookingId} уже начислен, пропускаем`);
    return data.worker as Worker;
  }

  // ✅ Создаем транзакцию только если реально начислили
  let description = `Заказ #${bookingId.slice(0, 8)}`;
  if (worker.working_mode === 'solo') {
    description += ' (solo)';
  } else if (worker.working_mode === 'pair' && partnerName) {
    description += ` (Пара: ${partnerName})`;
  } else {
    description += ' (pair)';
  }
